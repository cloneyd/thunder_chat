...
class Content extends Component {
  ...
  ws = new WebSocket("wss://" + wsHost + "/ws/chat/" + this.chatId + "/");
    ...
    return (
      <div className="Content">
        <div className="info">
          ...
          <Link to="/video-call" id="call-button">
        ...
        <div id="chat">
          {this.state.messages.length ? (
            this.state.messages.map((item, index) => {
              const other =
                item.username !== this.state.username ? "other" : "";
              return (
                <Message
                  key={index}
                  other={other}
            ...
            <p id="no-messages">
              There is no messages right now. Be the first and start the
              conversation!{" "}
            </p>
          )}
        ...
        <form className="message-input" onSubmit={this.handleSendMessage}>
          ...
          <input
            ...
            onChange={(event) =>
              this.setState({ ...this.state, textMessage: event.target.value })
            }
